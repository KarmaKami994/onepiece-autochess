...
function sanitizeResolvableCarouselChoices(
  ...
  match.carouselChoices = match.carouselChoices.flatMap((rawChoice, index) => {
    const choice: unknown = rawChoice;
    ...
    const takenByPlayerId =
      choice.takenByPlayerId === null || typeof choice.takenByPlayerId === "string"
        ? choice.takenByPlayerId
        : null;
    if (!id || !itemId || !itemIds.has(itemId)) return [];
    return [{
      id,
      itemId,
      takenByPlayerId,
      orbitIndex: Number.isInteger(choice.orbitIndex)
        ...
        : index,
      claimedAtTick:
        choice.claimedAtTick === null || Number.isInteger(choice.claimedAtTick)
...
function hasValidCarouselCheckpoint(
  ...
  if (
    !Number.isInteger(session.tick) || session.tick < 0 ||
    !Number.isInteger(session.durationTicks) || session.durationTicks <= 0 ||
    !Number.isInteger(session.arenaSeed) ||
    (session.finishAtTick !== null &&
      (!Number.isInteger(session.finishAtTick) || session.finishAtTick < 0)) ||
    !Array.isArray(session.participants) || !Array.isArray(session.events)
  ) {
    return false;
  }
  ...
  for (const participant of session.participants) {
    if (
      ...
      !Number.isInteger(participant.rank) || participant.rank < 1 ||
      !isPosition(participant.spawnPosition) ||
      !isPosition(participant.position) ||
      !isPosition(participant.targetPosition) ||
      !Number.isInteger(participant.releaseTick) || participant.releaseTick < 0 ||
  ...
  if (participantIds.size !== livingPlayerIds.size) return false;

  const itemIds = new Set(content.items.map((item) => item.id));
  const choiceIds = new Set<string>();
  for (const choice of match.carouselChoices) {
    if (
      typeof choice.id !== "string" || choiceIds.has(choice.id) ||
      typeof choice.itemId !== "string" || !itemIds.has(choice.itemId) ||
      !(choice.takenByPlayerId === null || participantIds.has(choice.takenByPlayerId)) ||
      !Number.isInteger(choice.orbitIndex) || choice.orbitIndex < 0 ||
  ...
  if (match.carouselChoices.length < session.participants.length) return false;
  for (const participant of session.participants) {
    ...
    if (
      ...
      match.carouselChoices.some(
        (choice) => choice.takenByPlayerId === participant.playerId &&
  ...
  return session.events.every(
    (event) => isRecord(event) && typeof event.id === "string" &&
      typeof event.type === "string" && Number.isInteger(event.tick) &&
      Number(event.tick) >= 0,
  );
}
...
export function migrateMatchState(
  ...
  if (version <= 1) {
    ...
    mutable.stageId = typeof mutable.stageId === "string"
      ? mutable.stageId
    ...
    mutable.carouselChoices = Array.isArray(mutable.carouselChoices)
      ...
      : [];
    mutable.nextChoiceSerial = typeof mutable.nextChoiceSerial === "number"
  ...
  if (version <= 4 && Array.isArray(mutable.players)) {
    for (const rawPlayer of mutable.players) {
      if (isRecord(rawPlayer)) rawPlayer.recentBattles = [];
    }
  }
  ...
  for (const field of ["players", "pairings", "lastResults"] as const) {
  ...
  if (version <= 3) {
    migrated = migrated.phase === "battle"
  ...
  for (const player of migrated.players) {
    if (isRecord(player.units)) {
      for (const instance of Object.values(player.units)) {
        reconcileProductionFormProgression(instance, content);
      }
    }
...
export function serializeMatch(state: MatchState, savedAt = "local"): string {
...
export function deserializeMatch(
  ...
  return migrateMatchState("match" in parsed ? parsed.match : parsed, content);
}

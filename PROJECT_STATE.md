...
PR #30 is merged at `76609be`, establishing GameContent `1.15.1` with Smoker at White Blow 180 and 1400ms attack cadence. PR #31 is an analysis-only residual-advantage decision review on `analysis/smoker-residual-advantage-review`; no gameplay change or new simulation is included.

## Last Completed Work

- 2026-08-31 — PR #31 analysis on `analysis/smoker-residual-advantage-review`: reviewed the four committed Smoker snapshots, current 2-cost peers, targeted kits, Navy/Guardian/Straw Hat reachability and line/adjacent geometry without changing gameplay or running a simulation. Decision: C — targeted diagnostics first. Smoker remains strongly above the full band but only modestly above Franky (+2.16 pp top four, +3.67 pp conditional wins, 0.152 better placement), while Franky has stronger raw stats and higher measured cast, target, damage and control expression. Aggregate evidence cannot separate intrinsic Smoker strength from Navy/Guardian composition and selective retention. Exactly one next task is an analysis-only Smoker outcome split across neither/Navy-only/Guardian-only/both active-trait states. GameContent remains `1.15.1`; save schema remains 6. Material files: `docs/SMOKER_RESIDUAL_ADVANTAGE_REVIEW.md`, `PROJECT_STATE.md`.
...
- PR #31 classification is C — targeted diagnostics first. No third Smoker balance change is justified from aggregate evidence. Preserve White Blow 180, 1400ms cadence, line, knockback, Navy and Guardian until one analysis-only diagnostic reports Smoker final-board outcomes across neither/Navy-only/Guardian-only/both active-trait states.
...
- The post-forms assessment is classification B — specific balance follow-up warranted. Persistent-form final-board statistics use the same last deployed `player.board` observation as base-character presence and never count bench units; this is an ADAPTED PORT of PAC final-team statistics semantics while preserving the local separate board/bench model. Form-aware combat diagnostics retain `definitionId` as economic identity, use real non-ghost snapshots, follow the recorded transform timeline, and leave historical aggregates unchanged. The sole next bounded task is an isolated Smoker adjustment; Chopper remains a secondary watch and Gear 4 requires more observations.
...
One analysis-only Smoker diagnostic: report final-board top-four rate, conditional win rate and average placement for boards containing Smoker, stratified by active trait state—neither Navy nor Guardian, Navy only, Guardian only, and both. Do not implement a balance change or combine it with other roster work.

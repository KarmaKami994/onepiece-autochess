...
PR #35 is merged at `26b0a75`, completing P3B deterministic global PvP pairing. Production-soak population normalization is implemented on `fix/production-soak-population` and awaiting review: all eight harness participants are bots before the first phase advancement, and the seven existing personalities rotate deterministically across slots and seeds. A new authoritative broad baseline is required after this PR merges. Captain damage, late PAC PvE/event cadence and Smoker remain unchanged or deferred; GameContent remains `1.15.1` and save schema remains 6.

## Last Completed Work

- 2026-09-01 — Production-soak population normalization on `fix/production-soak-population`: the harness now assigns all eight participants as bots before the first phase advancement and rotates the seven existing personality IDs with `(seedIndex + playerIndex) % personalityIds.length`. Each match contains all seven personalities plus one rotating duplicate; across seven seeds each personality has eight assignments and each player slot uses every personality once. The redundant per-transition `player-1` bot reassertion is removed. Normal `createMatch()`, bot policy, gameplay, metrics, RNG, GameContent and schema are unchanged. Historical snapshots retain their exact-harness conclusions; a new authoritative broad baseline is required after merge. Material files: `scripts/run_production_soak.ts`, `tests/production-audit.test.ts`, `docs/BOT_ARCHITECTURE_AND_BIAS_AUDIT.md`, `PROJECT_STATE.md`.
...
- The pre-fix production soak had a confirmed measurement-harness population defect: although `player-1` was already converted before the first preparation transition, assigning it Balanced permanently duplicated that personality in every match. The task branch resolves this with deterministic rotating assignment across all eight bot slots. Historical results remain valid exact-historical-harness evidence; a new authoritative broad baseline is required after merge and was not generated here.
...
- Roadmap is P2/P3 complete → merge the production-soak population normalization → run a new authoritative broad baseline → resume only separately approved P1B/P4/economy/balance work. Smoker remains frozen with no immediate diagnostic.
...
- Production-soak population policy uses only the seven existing content personalities and rotates assignments across the eight fixed player slots by seed. A new authoritative broad baseline must follow after this PR merges; historical snapshots are not rewritten. Smoker remains frozen as a watch item.
...
Await review and merge of `fix/production-soak-population`. After merge, run a separately authorized new authoritative broad baseline. Do not start P1B, P4, captain-damage or balance work automatically.
